...
if __name__ == "__main__":
    ...
    epd = EPD1IN54(rotation=180, spi=epdSPI, cs_pin=Pin(7), dc_pin=Pin(5), reset_pin=p, busy_pin=Pin(16))   #epd.clear()
    
    epd.draw.text("J'aime ca les patates!", smol, 0, 140)
    epd.draw.text('19', big, 0, 0, c = 1)
    epd.show(True,key = 0)
    epd.sleep()

    epd.reinit()
    epd(2, True, True, True)
    epd.draw.text('19', big, 0, 0, diff=True, c=1)
    epd.draw.text('WW', big, 0, 0, c=1)
    epd.show(True)
    epd.sleep()
